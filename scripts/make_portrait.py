...
DETAIL = 0.0               # deepen dark detail; see prep(). 0 = off
DETAIL_SIGMA = 4.0         # scale of "detail", in pixels of the input
...
def prep(path, crop=None, clahe=CLAHE_CLIP, curve=CURVE, detail=DETAIL,
         upscale=1):
    """Cut out the background, even the local contrast, then darken.

    Note the order: the background is cut from the *whole* photo, before the
    crop. u2net segments a figure, not a face — hand it a tight head crop and
    the mask comes back close to useless, the matte never goes white, and the
    background fills the frame with mid-tone characters.
    """
    ...
    if crop:
        cut = cut.crop(crop)
    if upscale > 1:
        # A small head in a large photo leaves few pixels per character cell.
        # Upscaling adds no detail, but it does give CLAHE's tiles and the
        # detail pass something to work on other than single pixels.
        cut = cut.resize((cut.width * upscale, cut.height * upscale),
                         Image.LANCZOS)
    ...
    gray = cv2.createCLAHE(clipLimit=clahe,
                           tileGridSize=(8, 8)).apply(gray).astype("float32")
    if detail:
        # Deepen only what is already darker than its surroundings — eyes,
        # brows, lips, nostrils. Side light supplies that contrast for free;
        # a flatly lit photo has none, so this manufactures some. It is not a
        # substitute for the light, only a partial rescue.
        local = cv2.GaussianBlur(gray, (0, 0), DETAIL_SIGMA)
        gray = gray - detail * np.clip(local - gray, 0, None)
...
def main():
    ...
    ap.add_argument("--curve", type=float, default=CURVE,
                    help="darkening exponent; 1.7 suits side light, a flatly "
                         "lit photo needs about 1.0 or it fills in solid")
    ap.add_argument("--detail", type=float, default=DETAIL,
                    help="deepen dark detail (eyes, brows, lips) when the "
                         "light gave you no shadow to draw with; try 1.6")
    ap.add_argument("--upscale", type=int, default=1,
                    help="enlarge the crop first, for a head that is small in "
                         "the source photo")
    ...
    img = prep(args.photo, crop, clahe=args.clahe, curve=args.curve,
               detail=args.detail, upscale=args.upscale)
